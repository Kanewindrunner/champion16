...
//ver2            //puts("");
            ...
            predDir = brpred->GetPrediction(PC);//mark
            brpred->UpdatePredictor(PC, opType, branchTaken, predDir, branchTarget);//mark 
            //brpred->UpdatePredictor(branchTaken);//mark 
...
//ver2      printf("  MISPRED_PER_1K_INST_BTB_DYN \t : %10.4f",   1000.0*(double)(numMispred_btbDYN)/(double)(total_instruction_counter));
      printf("  Precise\t : %10.4f", 100-100.0*(double)(numMispred)/(double)(cond_branch_instruction_counter));	
      printf("\n");
}
